# ============================================================
#  TBH Box - host installer (called by install.bat)
...
try { Unregister-ScheduledTask -TaskName 'TBHBoxInstall' -Confirm:$false -ErrorAction Stop } catch { }

Write-Host '============================================'
Write-Host ' TBH Box - Steam + TBH: Task Bar Hero'
...
if ($rebootNeeded) {
    ...
    try {
        ...
        Register-ScheduledTask -TaskName 'TBHBoxInstall' -Action $action -Trigger $trigger -Principal $principal -Force | Out-Null
...
if (-not (Test-Path $envFile)) {
    ...
    @(
        '# TBH Box configuration (read by docker compose). Generated by the installer.',
...
Write-Host ''
Write-Host '  When the desktop appears:'
Write-Host '   1. The box installs Steam + the AI server automatically (wait ~2 min).'
Write-Host '   2. Open Steam and log in to YOUR account (normal Steam Guard).'
Write-Host '   3. Click the "Install TBH" shortcut on the box desktop.'
Write-Host ''
